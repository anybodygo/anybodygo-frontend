...
export default function Input({id, setFilters = f => f, clearFilter = f => f, filters}) {
    ...
    useThrottledEffect(()=> {
        if (input === '') {
            setOptions([
                {"type":"CityId","value":4400,"name":"Москва","parent":"Россия","flag":null},
  ...
  return (
    ...
    onFocus={()=> setShowOptions(true)} onBlur={(e)=>handleBlur(e)}>
        <input type='text' id={`${id}`} onChange={(e)=> setInput(e.target.value)} 
            className='filters-field filters-input' value={input} placeholder='Введите локацию'> 
        </input>
